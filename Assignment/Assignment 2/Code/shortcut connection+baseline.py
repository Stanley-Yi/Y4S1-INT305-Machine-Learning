...
def Net():
    ...
    # model.load_state_dict(torch.load('./model_result/shortcut connection+baseline/2021-11-22 13-37-27/20_97.07.pt'))
    return model
...
def plot_confusion_matrix(cm, classes, save_path, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.tight_layout()
    plt.title('Confusion matrix: shortcut connection+baseline')
...
def main():
    ...
    base_path = "./model_result/shortcut connection+baseline/"
    ...
    plt.title('loss/acc graph: shortcut connection+baseline')
    ...
    plt.tight_layout()
    plt.suptitle('Mis-classified example: shortcut connection+baseline')
    ...
    for key, value in well_classify.items():
        ...
        plt.xlabel('confidence: ' + str(round(float(value[1]), 2)))
    plt.tight_layout()
    plt.suptitle('Well-classified example: shortcut connection+baseline')
